...
CASE("structured-biquasicubic without limiter, rank 1") {
    bool limiter = false;
    int rank = 1;
    do_test(limiter, rank);
}
CASE("structured-biquasicubic with limiter, rank 1") {
    bool limiter = true;
    int rank = 1;
    do_test(limiter, rank);
}
CASE("structured-biquasicubic without limiter, rank 2") {
    bool limiter = false;
    int rank = 2;
    do_test(limiter, rank);
}
CASE("structured-biquasicubic with limiter, rank 2") {
    bool limiter = true;
    int rank = 2;
    do_test(limiter, rank);
}
CASE("structured-biquasicubic without limiter, rank 3") {
    bool limiter = false;
    int rank = 3;
    do_test(limiter, rank);
}
CASE("structured-biquasicubic with limiter, rank 3") {
    bool limiter = true;
    int rank = 3;
    do_test(limiter, rank);
}
