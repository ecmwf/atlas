...
size_t eval_idx( size_t pos,
                 std::array<size_t, Rank>& strides, FirstDim first ) {
...
size_t eval_idx( size_t pos,
                 ...
                 SecondDim second ) {
    return static_cast<size_t>(first) * strides[pos] +
           eval_idx<Rank>( pos + 1, strides, second );
}
...
size_t eval_idx( size_t pos, std::array<size_t, Rank>& strides, FirstDim first,
                 SecondDim second, ThirdDim third ) {
    return static_cast<size_t>(first) * strides[pos] +
           eval_idx<Rank>( pos + 1, strides, second, third );
}
...
struct validate_impl<DATA_TYPE, Rank, 0> {
    ...
    static void apply( array::ArrayView<DATA_TYPE, Rank>& arrv,
                       DATA_TYPE arr_c[], std::array<size_t, Rank>& strides,
                       Int... dims ) {
        EXPECT(arrv( dims... ) ==
               arr_c[eval_idx<Rank>( static_cast<size_t>(0), strides, dims... )]);
    }
};
...
struct compute_strides<0> {
    ...
    static void apply( array::ArrayView<DATA_TYPE, Rank>& arrv,
                       std::array<size_t,
                       static_cast<unsigned int>(Rank)>& strides ) {}
};
...
struct compute_strides {
    ...
    static void apply( array::ArrayView<DATA_TYPE, Rank>& arrv,
                       std::array<size_t,
                       static_cast<unsigned int>(Rank)>& strides ) {
        strides[Dim - 1] = strides[Dim] *
            static_cast<unsigned int>(
            arrv.template shape<static_cast<unsigned int>(Dim)>());
...
struct validate {
    static void apply( array::ArrayView<DATA_TYPE, Rank>& arrv,
        ...
        for ( idx_t i = 0; i < arrv.template shape<0>(); ++i ) {
            validate_impl<DATA_TYPE, Rank, Rank - 1>::apply( arrv, arr_c,
                                                             strides, i );
        }
    }
};
...
void adjoint_test( POD sum1, POD sum2,
  ...
  std::cout << "Adjoint test " << string_test << " " << sum1 << " "
            << sum2 << std::endl;
...
void test_rank0_arrview_adj_test( Fixture& f ) {
    ...
    array::ArrayView<POD, 1> arrv_init =
      array::make_host_view<POD, 1>( arr_init );
    array::ArrayView<POD, 1> arrv =
      array::make_host_view<POD, 1>( arr_init );

    for ( std::size_t j = 0; j < static_cast<std::size_t>(f.N); ++j ) {
        arrv_init( j ) =
          (static_cast<std::size_t>(f.part[j]) !=
           mpi::comm().rank() ? 0 : f.gidx[j] );
        arrv( j ) = arrv_init( j );
    }
...
void test_rank1( Fixture& f ) {
    ...
    array::ArrayView<POD, 2> arrv =
      array::make_host_view<POD, 2>( arr );
    ...
    switch ( mpi::comm().rank() ) {
        ...
        case 2: {
            POD arr_c[] =
                {0, 0, 0, 0, 140 , 1400, 160, 1600, 180, 1800, 0, 0, 0, 0};
            validate<POD, 2>::apply( arrv, arr_c );
            break;
        }
    }
...
void test_rank1_adj_test( Fixture& f ) {
    ...
    array::ArrayT<POD> arr( f.N, 2 );
    array::ArrayView<POD, 2> arrv_init =
...
void test_rank1_strided_v1_adj_test( Fixture& f ) {
    ...
    array::ArrayView<POD, 2> arrv_init_t =
      array::make_host_view<POD, 2>( arr_init_t );
    array::ArrayT<POD> arr_t( f.N, 2 );
    array::ArrayView<POD, 2> arrv_t =
            ...
            for (std::size_t j = 0; j < static_cast<size_t>(f.N); ++j ) {
                for (std::size_t i = 0; i < 3; ++i ) {
                    for (std::size_t k = 0; k < 2; ++k) {
                        arrv(j, i, k) =  ( 2 * static_cast<int>(k) -1) *
                                           arr_c[j] * std::pow(10, i );
                    }
                }
            }
            break;
...
void test_rank2_adj_test( Fixture& f ) {
    ...
    array::ArrayView<POD, 3> arrv_init =
        array::make_host_view<POD, 3>( arr_init );
    array::ArrayT<POD> arr( f.N, 3, 2 );
    array::ArrayView<POD, 3> arrv =
        array::make_host_view<POD, 3>( arr );

    for (size_t p = 0; p < static_cast<size_t>(f.N); ++p ) {
        for ( size_t i = 0; i < 3; ++i ) {
            arrv_init( p, i, static_cast<size_t>(0) ) =
                ( static_cast<size_t>( f.part[p] ) !=
                  mpi::comm().rank() ? 0 : -f.gidx[p] * std::pow( 10, i ) );
            arrv_init( p, i, static_cast<size_t>(1)) =
                ( static_cast<size_t>( f.part[p] ) !=
                  mpi::comm().rank() ? 0 : f.gidx[p] * std::pow( 10, i ) );
            arrv( p, i, static_cast<size_t>(0) ) =
                  arrv_init( p, i, static_cast<size_t>(0) );
            arrv( p, i, static_cast<size_t>(1) ) =
                  arrv_init( p, i, static_cast<size_t>(1) );
        }
    }
    ...
    for ( std::size_t p = 0; p < static_cast<std::size_t>(f.N); ++p ) {
        for ( std::size_t i = 0; i < 3; ++i ) {
            arrv_init_t( p, i, static_cast<std::size_t>(0) ) =
                (static_cast<std::size_t>( f.part[p] ) !=
                 mpi::comm().rank() ? 0 : -f.gidx[p] * std::pow( 10, i ) );
            arrv_init_t( p, i, static_cast<std::size_t>(1) ) =
                (size_t( f.part[p] ) !=
                 mpi::comm().rank() ? 0 : f.gidx[p] * std::pow( 10, i ) );
            arrv_t( p, i, static_cast<std::size_t>(0) ) =
                arrv_init_t( p, i, static_cast<std::size_t>(0) );
            arrv_t( p, i, static_cast<std::size_t>(1) ) =
                arrv_init_t( p, i, static_cast<std::size_t>(1) );
        }
    }
    arr_t.syncHostDevice();
...
void test_rank2_l2_v2( Fixture& f ) {
   ...
   */
    ...
    switch ( mpi::comm().rank() ) {
        ...
        case 1: {
            POD arr_c[] = {-3, 3, -30,   0, -300, 300,  // halo
                           -4, 4, -40,  80, -400, 400,  // core
                           -5, 5, -50, 100, -500, 500,  // core
                           -6, 6, -60, 120, -600, 600,  // core
                           -7, 7, -70,   0, -700, 700   // halo
                           -8, 8, -80,   0, -800, 800}; // halo
            validate<POD, 3>::apply( arrv_t, arr_c );
            break;
        }
        case 2: {
...
void test_rank0_wrap_adj_test( Fixture& f ) {
    ...
    std::unique_ptr<array::Array> arr(
         array::Array::wrap<POD>( f.gidx.data(), array::make_shape( f.N ) ) );
    array::ArrayView<POD, 1> arrv =
         array::make_view<POD, 1>( *arr );
    ...
    switch ( mpi::comm().rank() ){
        case 0: {
            ...
            for (std::size_t j = 0; j < static_cast<size_t>(f.N); ++j ) {
                for (std::size_t i = 0; i < 3; ++i ) {
                    for (std::size_t k = 0; k < 2; ++k) {
                        arrv(i, j, k) =  (2 * static_cast<int>(k) - 1) *
                                         arr_c[j] * std::pow(10, i );
                    }
                }
            }
    ...
    for ( size_t p = 0; p < static_cast<size_t>(f.N); ++p ) {
        for ( size_t i = 0; i < 3; ++i ) {
            arrv_init( i, p, static_cast<size_t>(0) ) =
                ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 :
                  -f.gidx[p] * std::pow( 10, i ) );
            arrv_init( i, p, static_cast<size_t>(1) ) =
                ( size_t( f.part[p] ) != mpi::comm().rank() ? 0 :
                  f.gidx[p] * std::pow( 10, i ) );
            arrv(i, p, static_cast<size_t>(0) ) =
                arrv_init( i, p, static_cast<size_t>(0) );
            arrv(i, p, static_cast<size_t>(1) ) =
                arrv_init( i, p, static_cast<size_t>(1) );
        }
    }
...
*/
    ...
    SECTION( "test_rank1_paralleldim_1_adj_test" )
         { test_rank1_paralleldim1_adj_test( f ); }
