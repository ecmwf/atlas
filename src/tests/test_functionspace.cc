...
{
  ScopedPtr<Grid> grid( Grid::create("oct.N16") );

  Mesh mesh(*grid);

  size_t nb_nodes = mesh.function_space("nodes").shape(0);
  size_t nb_levels = 10;

  NodesFunctionSpace nodes_fs("nodes",mesh);
  NodesColumnFunctionSpace columns_fs("columns",mesh,nb_levels);

  ScopedPtr<Field> surface_scalar_field( nodes_fs.create_field<double>("scalar") );
  ScopedPtr<Field> surface_vector_field( nodes_fs.create_field<double>("vector",2) );
  ScopedPtr<Field> surface_tensor_field( nodes_fs.create_field<double>("tensor",2,2) );
  ...
  BOOST_CHECK_EQUAL( surface_scalar_field->size() , nb_nodes );
  BOOST_CHECK_EQUAL( surface_vector_field->size() , nb_nodes*2 );
  BOOST_CHECK_EQUAL( surface_tensor_field->size() , nb_nodes*2*2 );
  ...
  size_t surface_scalar_shape[] = { nb_nodes };
  size_t surface_vector_shape[] = { nb_nodes, 2 };
  size_t surface_tensor_shape[] = { nb_nodes, 2, 2 };
  ...
  ScopedPtr<Field> columns_scalar_field( columns_fs.create_field<double>("scalar") );
  ScopedPtr<Field> columns_vector_field( columns_fs.create_field<double>("vector",2) );
  ScopedPtr<Field> columns_tensor_field( columns_fs.create_field<double>("tensor",2,2) );
  ...
  BOOST_CHECK_EQUAL( columns_scalar_field->size() , nb_nodes*nb_levels );
  BOOST_CHECK_EQUAL( columns_vector_field->size() , nb_nodes*nb_levels*2 );
  BOOST_CHECK_EQUAL( columns_tensor_field->size() , nb_nodes*nb_levels*2*2 );
  ...
  size_t columns_scalar_shape[] = { nb_nodes, nb_levels };
  size_t columns_vector_shape[] = { nb_nodes, nb_levels, 2 };
  size_t columns_tensor_shape[] = { nb_nodes, nb_levels, 2, 2 };
...
BOOST_AUTO_TEST_CASE( test_SpectralFunctionSpace )
{
  ...
  ScopedPtr<Field> surface_scalar_field( spectral_fs.create_field<double>("scalar") );
