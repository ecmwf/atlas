...
Mesh getMesh( const FunctionSpace& functionSpace ) {
    ...
    if ( cellColumnsPtr ) {
        ...
        mesh = MeshGenerator( "structured" ).generate(
            structuredColumnsPtr->grid(), structuredColumnsPtr->distribution() );
    }
    return mesh;
}
...
CASE( "Structured grid" ) {
    ...
    SECTION( "Structured Columns" ) {
       ...
       const auto sourceFunctionSpace = functionspace::StructuredColumns( grid,
            grid::Partitioner( "equal_regions" ), util::Config( "halo", 2 ) |
                                                  util::Config( "periodic_points", true ) );
       const auto targetFunctionSpace = functionspace::StructuredColumns( grid,
            grid::Partitioner( "regular_bands" ), util::Config( "halo", 2 ) |
                                                  util::Config( "periodic_points", true ) );
...
CASE( "Cubed sphere grid" ) {
    ...
    const auto sourceMeshConfig = util::Config( "partitioner", "equal_regions" ) |
                                  util::Config( "halo", "2" );
    const auto targetMeshConfig = util::Config( "partitioner", "cubedsphere" ) |
                                  util::Config( "halo", "2" );
