        ...
        if ( mpi_rank == r ) {
            ...
            if ( mpi_rank == 0 ) {  // replace "r" with rank you wish to plot only
                ...
                f << "\n"   "])"
                     ...
                     "\n"   ""
                     "\n"   "plt.spy( matrix_"<<r<<",markersize=1 )"
                     "\n"   "";
            }
            if ( mpi_rank == mpi_size - 1 ) {
                f << "\n"   "plt.grid()"
...
Grid grid() {
    return Grid{eckit::Resource<std::string>( "--grid", "O16" )};
}

void test_reordering( const util::Config& reorder_config, const std::vector<int>& expected = {} ) {
    ...
    outputConnectedCoordinates( type + "_nodes.py", xy );
    ...
    outputConnectedCoordinates( type + "_elements.py", cell_centres_xy );

    Field edge_centres   = create_edge_centres( mesh );
    auto edge_centres_xy = array::make_view<double, 2>( edge_centres );
    outputConnectedCoordinates( type + "_edges.py", edge_centres_xy );
    ...
    outputTriplets( type + "_n2n_triplets.py", n2n_triplets, mesh.nodes().size(), mesh.nodes().size() );
    outputTriplets( type + "_e2n_triplets.py", e2n_triplets, mesh.edges().size(), mesh.nodes().size() );
}
...
CASE( "test_hilbert_reordering" ) {
    ...
    if ( grid().name() == "O16" && mpi::comm().size() == 1 ) {
        // clang-format off
