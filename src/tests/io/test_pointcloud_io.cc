...
CASE( "write_read_write_field" )
{
  ...
  {
    EXPECT( eckit::types::is_approximately_equal(funny_formula(i), field_data(i), 0.001));  // 0.001% relative error
  }
  ...
  {
    EXPECT( eckit::types::is_approximately_equal(field_data(i), field_from_FieldSet_data(i), 0.001));  // 0.001% relative error
