...
module ftnunit
    ...
    interface assert_comparable
        module procedure assert_comparable_real4
        module procedure assert_comparable_real8
...
subroutine runtests_final
    if ( ftnunit_file_exists("ftnunit.run") ) then
        write(0,'(a,i5)') 'Number of failed assertions:                ', nofails
        write(0,'(a,i5)') 'Number of runs needed to complete the tests:', noruns
        call ftnunit_remove_file( "ftnunit.lst" )
        write(0,'(a)',advance='no') 'exit '
        flush(unit=0)
        if( nofails > 0 .or. noruns > 1 ) then
          stop 1
        else
          stop 0
        end if
...
subroutine assert_true( cond, text )
    ...
    if ( .not. cond ) then
        nofails = nofails + 1
        write(0,*) '    Condition "',trim(text), '" failed'
        write(0,*) '    It should have been true'
    endif
...
subroutine assert_false( cond, text )
    ...
    if ( cond ) then
        nofails = nofails + 1
        write(0,*) '    Condition "',trim(text), '" failed'
        write(0,*) '    It should have been false'
    endif
...
subroutine assert_equal_int( value1, value2, text )
    ...
    if ( value1 /= value2) then
        nofails = nofails + 1
        write(0,*) '    Values not equal: "',trim(text), '" - assertion failed'
        write(0,*) '    Values: ', value1, ' and ', value2
    endif
end subroutine assert_equal_int

! assert_equal_int --
!     Subroutine to check if two logicals are equal
! Arguments:
!     value1        First value
!     value2        Second value
!     text          Text describing the assertion
...
subroutine assert_equal_int1d( array1, array2, text )
    ...
    if ( size(array1) /= size(array2) ) then
        nofails = nofails + 1
        write(0,*) '    Arrays have different sizes: "',trim(text), '" - assertion failed'
    else
        if ( any( array1 /= array2 ) ) then
            nofails = nofails + 1
            write(0,*) '    One or more values different: "',trim(text), '" - assertion failed'
            count = 0
            do i = 1,size(array1)
                if ( array1(i) /= array2(i) ) then
                    count = count + 1
                    write(0,'(3a10)')    '    Index', '     First', '    Second'
                    if ( count < 50 ) then
                        write(0,'(3i10)')    i, array1(i), array2(i)
                    endif
                    write(0,*) 'Number of differences: ', count
                endif
            enddo
        endif
    endif
...
!     output. Also, nofails is increased by one.
!
subroutine assert_comparable_real4( value1, value2, margin, text )
    real(kind=4), intent(in)             :: value1
    real(kind=4), intent(in)             :: value2
    real(kind=4), intent(in)             :: margin
    character(len=*), intent(in) :: text

    if ( abs(value1-value2) > 0.5 * margin * (abs(value1)+abs(value2)) ) then
        nofails = nofails + 1
        write(0,*) '    Values not comparable: "',trim(text), '" - assertion failed'
        write(0,*) '    Values: ', value1, ' and ', value2
    endif
end subroutine assert_comparable_real4
...
!     output. Also, nofails is increased by one.
!
subroutine assert_comparable_real8( value1, value2, margin, text )
...
end subroutine assert_comparable_real8
        ...
        if ( any( abs(array1-array2) > 0.5 * margin * (abs(array1)+abs(array2)) ) ) then
            ...
            do i = 1,size(array1)
                if ( abs(array1(i)-array2(i)) > &
                    ...
                    count = count + 1
                    write(0,'(a10,2a15)')    '    Index', '          First', '         Second'
                    if ( count < 50 ) then
                        write(0,'(i10,e15.5)')    i, array1(i), array2(i)
                    endif
                    write(0,*) 'Number of differences: ', count
                endif
            enddo
...
subroutine ftnunit_remove_file( filename )
    ...
    if ( ierr /= 0 ) then
        write(0,*) '    Could not open file for removal: ', trim(filename)
        nofails = nofails + 1
    else
        close( lun, status = 'delete' )
        if ( ftnunit_file_exists( filename ) ) then
            write(0,*) '    Removal of file unsuccssful: ', trim(filename)
            nofails = nofails + 1
        endif
    endif
...
subroutine ftnunit_make_empty_file( filename )
    ...
    if ( ierr /= 0 ) then
        write(0,*) '    Failed to create empty file: ', trim(filename)
        nofails = nofails + 1
    else
        close( lun )
    endif
