...
void read_file( const eckit::PathName& p, std::ostream& out ) {
    if ( p.exists() ) {
        ...
        if ( !in ) { throw eckit::CantOpenFile( p.asString(), Here() ); }
        else {
            out << in.rdbuf();
            in.close();
        }
...
int atlas__read_file( const char* path, char*& content, int& size ) {
    ATLAS_ERROR_HANDLING(
        ...
        std::stringstream ss; atlas::read_file( path, ss ); std::string s = ss.str(); size = s.size() + 1;
        content = new char[size]; strcpy( content, s.c_str() ); return true; );
    return false;
}
