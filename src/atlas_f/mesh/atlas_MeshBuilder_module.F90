...
function atlas_TriangularMeshBuilder__build(this, &
  ...
  mesh = atlas_Mesh( atlas__TriangularMeshBuilder__operator(this%CPTR_PGIBUG_A, &
       ...
       & array_view1d(x),   array_view1d(y),   int(array_stride(x,1),c_size_t),   int(array_stride(y,1),c_size_t),   &
       & array_view1d(lon), array_view1d(lat), int(array_stride(lon,1),c_size_t), int(array_stride(lat,1),c_size_t), &
       & int(nb_triags,c_size_t), array_view1d(triag_global_index), array_view1d(triag_nodes), &
       & global_index_base_ ))
  call mesh%return()
end function
