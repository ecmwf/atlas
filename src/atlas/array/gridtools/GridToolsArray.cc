...
public:
    ...
    switch (shape.size()) {
      ...
      case 4: {
        if( layout[0]==0 && layout[1]==1 && layout[2] == 2 && layout[3] == 3)
          return construct_with_layout<::gridtools::layout_map<0,1,2,3>>(shape[0],shape[1],shape[2],shape[3]);
        if( layout[0]==3 && layout[1]==2 && layout[2] == 1 && layout[3] == 0)
          return construct_with_layout<::gridtools::layout_map<3,2,1,0>>(shape[0],shape[1],shape[2],shape[3]);
      }
      case 5: {
        if( layout[0]==0 && layout[1]==1 && layout[2] == 2 && layout[3] == 3 && layout[4] == 4)
          return construct_with_layout<::gridtools::layout_map<0,1,2,3,4>>(shape[0],shape[1],shape[2],shape[3],shape[4]);
        if( layout[0]==4 && layout[1]==3 && layout[2] == 2 && layout[3] == 1 && layout[4] == 0)
          return construct_with_layout<::gridtools::layout_map<4,3,2,1,0>>(shape[0],shape[1],shape[2],shape[3],shape[4]);
      }
      default: {
        std::stringstream err;
        if( shape.size() > 5 )
  ...
  void resize_variadic(Ints... c) {
      if(sizeof...(c) != array_.rank()){
          std::stringstream err; err <<
              "Trying to resize an array of Rank " << array_.rank() <<
              " by dimensions with Rank " << sizeof...(c) << std::endl;
          throw eckit::BadParameter(err.str(),Here());
      }
...
Array* Array::create(DataType datatype, const ArrayShape& shape, const ArrayLayout& layout)
{
  switch( datatype.kind() )
  {
  case DataType::KIND_REAL64: return create<double>(shape,layout);
  case DataType::KIND_REAL32: return create<float>(shape,layout);
  case DataType::KIND_INT32:  return create<int>(shape,layout);
  case DataType::KIND_INT64:  return create<long>(shape,layout);
  case DataType::KIND_UINT64: return create<unsigned long>(shape,layout);
...
void ArrayT<Value>::resize(const ArrayShape& shape)
{
    assert(shape.size() > 0);
    switch (shape.size()) {
        case 1: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<1>());
        case 2: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<2>());
        case 3: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<3>());
        case 4: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<4>());
        case 5: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<5>());
        case 6: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<6>());
        case 7: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<7>());
        case 8: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<8>());
        case 9: return ArrayT_impl<Value>(*this).apply_resize(shape, UintSequence<9>());
        default: {
            std::stringstream err;
            err << "shape not recognized";
            throw eckit::BadParameter(err.str(), Here());
