...
make_gt_host_view(const Array& array) {
  ...
  using storage_info_ty = storage_traits::storage_info_t<0, NDims>;
...
make_gt_device_view(const Array& array) {
  typedef storage_traits::storage_info_t<0, NDims> storage_info_ty;
...
make_host_view(const Array& array) {
  ...
  return ArrayView<Value, NDims>(gridtools::make_gt_host_view<Value, NDims>(array), array);
}
