...
public:
  ...
  TableIndex& operator=(const TableIndex& other) { set(other.get()); return *this; }
  TableIndex& operator+(const idx_t& value) { *(idx_)+=value; return *this; }
  TableIndex& operator-(const idx_t& value) { *(idx_)-=value; return *this; }
  TableIndex& operator--() { --(*(idx_)); return *this; }
  TableIndex& operator++() { ++(*(idx_)); return *this; }
  TableIndex& operator+=(const idx_t& value) { *(idx_)+=value; return *this; }
  TableIndex& operator-=(const idx_t& value) { *(idx_)-=value; return *this; }
...
public:
    ...
    // TODO: Following function should only be allowed to compile if ReadOnly=false (SFINAE?)
...
public:
  ...
  template<typename ReturnType> struct Access_t<ReturnType,true> {
    Access_t(const TableView<ReadOnly>* tv) : tv_(const_cast<TableView<ReadOnly>*>(tv)) {}
    TableView<ReadOnly>* tv_;
    idx_t apply(size_t row, size_t col) const {
        return tv_->values_(tv_->displs_(row) + col) FROM_FORTRAN;
    }
  };

  template<typename ReturnType> struct Access_t<ReturnType,false> {
    Access_t(const TableView<ReadOnly>* tv) : tv_(const_cast<TableView<ReadOnly>*>(tv)) {}
    TableView<ReadOnly>* tv_;
    Index apply(size_t row, size_t col) const {
