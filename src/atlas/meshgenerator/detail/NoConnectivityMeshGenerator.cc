...
NoConnectivityMeshGenerator::NoConnectivityMeshGenerator() {}

NoConnectivityMeshGenerator::NoConnectivityMeshGenerator( const eckit::Parametrisation& ) {}

NoConnectivityMeshGenerator::~NoConnectivityMeshGenerator() {}

void NoConnectivityMeshGenerator::hash( eckit::Hash& h ) const {
    h.add( "NoConnectivity" );

    // no other settings
}

void NoConnectivityMeshGenerator::generate( const Grid& grid, const grid::Distribution& dist, Mesh& mesh ) const {
    if ( dist.nb_partitions() > 1 ) {
        Log::warning() << "NoConnectivity mesh does not support a GridDistribution"
...
void NoConnectivityMeshGenerator::generate( const Grid& g, Mesh& mesh ) const {
    createNodes( g, mesh );
    ...
    mesh::actions::BuildNoConnectivityMesh()( mesh );

    setGrid( mesh, g, "serial" );
}

void NoConnectivityMeshGenerator::createNodes( const Grid& grid, Mesh& mesh ) const {
...
namespace {
static MeshGeneratorBuilder<NoConnectivityMeshGenerator> __no_connectivity( "no_connectivity" );
}
