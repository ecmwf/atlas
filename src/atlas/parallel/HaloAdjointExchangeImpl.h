...
struct halo_zeroer_impl {
    ...
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* send_buffer,
                                         Idx... idxs ) {
        for ( idx_t i = 0; i < field.template shape<CurrentDim>(); ++i ) {
            halo_zeroer_impl<ParallelDim, Cnt - 1, CurrentDim + 1>::apply( buf_idx, node_idx, field, send_buffer,
                                                                           idxs..., i );
        }
    }
};
...
struct halo_zeroer_impl<ParallelDim, Cnt, ParallelDim> {
    ...
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* send_buffer,
                                         Idx... idxs ) {
        halo_zeroer_impl<ParallelDim, Cnt - 1, ParallelDim + 1>::apply( buf_idx, node_idx, field, send_buffer, idxs...,
                                                                        node_idx );
    }
};
...
template <int ParallelDim, int Cnt, int CurrentDim>
struct halo_zeroer2_impl {
    template <typename DATA_TYPE, int RANK, typename... Idx>
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* recv_buffer,
                                         Idx... idxs ) {
        for ( idx_t i = 0; i < field.template shape<CurrentDim>(); ++i ) {
            halo_zeroer2_impl<ParallelDim, Cnt - 1, CurrentDim + 1>::apply( buf_idx, node_idx, field, recv_buffer,
                                                                           idxs..., i );
        }
    }
};
...
template <int ParallelDim>
struct halo_zeroer2_impl<ParallelDim, 0, ParallelDim> {
    template <typename DATA_TYPE, int RANK, typename... Idx>
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* recv_buffer,
                                         Idx... idxs ) {
...
template <int ParallelDim, int Cnt>
struct halo_zeroer2_impl<ParallelDim, Cnt, ParallelDim> {
    template <typename DATA_TYPE, int RANK, typename... Idx>
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* recv_buffer,
                                         Idx... idxs ) {
        halo_zeroer2_impl<ParallelDim, Cnt - 1, ParallelDim + 1>::apply( buf_idx, node_idx, field, recv_buffer, idxs...,
                                                                        node_idx );
    }
};

template <int ParallelDim, int CurrentDim>
struct halo_zeroer2_impl<ParallelDim, 0, CurrentDim> {
    template <typename DATA_TYPE, int RANK, typename... Idx>
    ATLAS_HOST_DEVICE static void apply( idx_t& buf_idx, const idx_t node_idx,
                                         array::ArrayView<DATA_TYPE, RANK>& field, DATA_TYPE* recv_buffer,
                                         Idx... idxs ) {
