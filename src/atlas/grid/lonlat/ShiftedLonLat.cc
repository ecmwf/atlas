...
std::string ShiftedLonLat::grid_type_str() {
    return "shifted_lonlat.obsolete";
}
...
ShiftedLonLat::ShiftedLonLat(const eckit::Parametrisation& p) :
    LonLat(Shift::LON|Shift::LAT, domain::Domain::makeGlobal()) {
    setup(p);
}


ShiftedLonLat::ShiftedLonLat(const size_t N, const domain::Domain& dom) :
    LonLat(Shift::LON|Shift::LAT, dom) {
    LonLat::setup(N, dom);
}


ShiftedLonLat::ShiftedLonLat(const size_t nlon, const size_t nlat, const domain::Domain& dom) :
    LonLat(Shift::LON|Shift::LAT, dom) {
