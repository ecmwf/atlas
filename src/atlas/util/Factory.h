...
public:
    ...
    FactoryDeprecated(bool v) : value_(v) {
    }
    FactoryDeprecated() : value_(true) {
    }
    FactoryDeprecated(const char* message) : value_(true) {
        message_ = message; // assign(message.data(), message.size());
    }

//    FactoryDeprecated(const FactoryDeprecated& other) : value_(other.value_), message_(other.message_) {
//   }
