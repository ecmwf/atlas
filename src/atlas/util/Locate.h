...
inline void locate(const FunctionSpace& fs, std::size_t size, const gidx_t global_index[], int partition[], idx_t remote_index[], idx_t remote_index_base) {
    ...
    atlas::vector<int> partition_global(partition_global_size);
    ...
    auto assert_in_bounds = [](auto index, auto& vector) {
        if( index >= vector.size() ) {
            Log::error() << "Index " << index << " out of bounds; size = " << vector.size() << std::endl;
            throw_OutOfRange(index, vector.size());
        }
    };
