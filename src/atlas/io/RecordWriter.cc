...
size_t RecordWriter::write( Stream out ) const {
    ATLAS_TRACE( "RecordWriter::write" );
    RecordHead r;
    ...
    // Metadata section
    // ----------------
    ATLAS_TRACE_SCOPE( "metadata section" ) {
    ...
    // Data sections
    // -------------
    ATLAS_TRACE_SCOPE( "data sections" ) {
        size_t i{0};
        for ( auto& key : keys_ ) {
...
void RecordWriter::set( const RecordWriter::Key& key, Link&& link, const util::Config& ) {
