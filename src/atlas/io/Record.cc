...
Endian RecordHead::endian() const {
    ...
    throw_Exception( "Mixed endianness is not supported", Here() );
}
...
const Metadata& Record::metadata( const std::string& key ) const {
    if ( record_->items.find( key ) == record_->items.end() ) {
        ATLAS_THROW_EXCEPTION( "Record does not contain key \"" << key << "\"" );
    }
...
Record& Record::read( Stream& in, bool read_to_end ) {
    ...
    if ( computed_metadata_checksum.available() &&
         encoded_metadata_checksum.str() != computed_metadata_checksum.str() ) {
        std::stringstream err;
    ...
    ATLAS_ASSERT( r.metadata_format == "yaml" );
