...
void ConservativeSphericalPolygonInterpolation::do_setup(const FunctionSpace& src_fs, const FunctionSpace& tgt_fs) {
    ...
    if (compute_cache) {
        ...
        auto& src_points = sharable_data_->src_.points;
        auto& src_areas  = sharable_data_->src_.areas;
        ...
        src_areas.resize(n_spoints_);
        {
            ...
            if (src_cell_data_) {
                ...
                auto& src_node2csp = sharable_data_->src_.node2csp;
        ...
        auto& tgt_points = sharable_data_->tgt_.points;
        auto& tgt_areas  = sharable_data_->tgt_.areas;
        ...
        tgt_areas.resize(n_tpoints_);
        {
            ...
            if (tgt_cell_data_) {
                ...
                auto& tgt_node2csp = sharable_data_->tgt_.node2csp;
                ...
                for (idx_t tpt = 0; tpt < n_tpoints_; ++tpt) {
                    ...
                    for (idx_t isubcell = 0; isubcell < tgt_node2csp[tpt].size(); ++isubcell) {
                        ...
                        auto tgt_csp = get_csp_nodedata(subcell, tgt_mesh_, sharable_data_->tgt_);
                        const auto& t_csp = tgt_csp.polygon;
                        tgt_areas[tpt] += t_csp.area();
...
void ConservativeSphericalPolygonInterpolation::intersect_polygons(const MarkedPolygonArray& src_csp) {
    ...
    auto& tgt_csp_size = sharable_data_->tgt_.csp_size;
    ...
    if (validate_) {
        ATLAS_TRACE_SCOPE("compute and output worst target polygon coverage by its intersections with source polygons") {
            ...
            if (rank_over == mpi::rank()) {
                ...
                auto tgt_csp = get_tgt_csp(tcsp_id);
                dump_polygons_to_json(tgt_csp.polygon, pointsSameEPS, src_csp, tgt_iparam_[tcsp_id].csp_ids, polygon_intersection_folder, "worst_target_cell_overcover");
            }
            ATLAS_TRACE_MPI(ALLGATHER) {
            ...
            if (rank_under == mpi::rank()) {
                ...
                auto tgt_csp = get_tgt_csp(tcsp_id);
                dump_polygons_to_json(tgt_csp.polygon, pointsSameEPS, src_csp, tgt_iparam_[tcsp_id].csp_ids, polygon_intersection_folder, "worst_target_cell_undercover");
            }
        }
    }
}
...
ConservativeSphericalPolygonInterpolation::Triplets ConservativeSphericalPolygonInterpolation::compute_2nd_order_triplets() {
    ...
    if (tgt_cell_data_) {
        ...
        for (idx_t tcsp_id = 0; tcsp_id < data_->tgt_.csp_size; ++tcsp_id) {
            ...
            if (src_cell_data_) {
                for (idx_t i_scsp = 0; i_scsp < iparam.csp_ids.size(); ++i_scsp) {
                    const idx_t scell = csp_to_cell(iparam.csp_ids[i_scsp], data_->src_);
                ...
                for (idx_t i_scsp = 0; i_scsp < iparam.csp_ids.size(); ++i_scsp) {
                    ...
                    const idx_t snode = data_->src_.csp2node[scsp_id];
        ...
        for (idx_t tcsp_id = 0; tcsp_id < data_->tgt_.csp_size; ++tcsp_id) {
            ...
            for (idx_t i_scsp = 0; i_scsp < iparam.csp_ids.size(); ++i_scsp) {
                ...
                const idx_t scsp_id     = iparam.csp_ids[i_scsp];
                const idx_t spt         = src_cell_data_ ? csp_to_cell(scsp_id,data_->src_) : data_->src_.csp2node[scsp_id];
        ...
        for (idx_t tnode = 0; tnode < n_tpoints_; ++tnode) {
            ...
            for (idx_t isubcell = 0; isubcell < tgt_node2csp[tnode].size(); ++isubcell) {
                const idx_t tsubcell = tgt_node2csp[tnode][isubcell];
                ...
                if (src_cell_data_) {
                    for (idx_t icell = 0; icell < iparam.csp_ids.size(); ++icell) {
                        const idx_t scell = iparam.csp_ids[icell];
                        const auto src_neighbours = get_cell_neighbours(src_mesh_, scell, w_cell);
                        triplets_size += 2 * src_neighbours.size() + 1;
                    }
                }
                    ...
                    for (idx_t icell = 0; icell < iparam.csp_ids.size(); ++icell) {
                        const idx_t subcell = iparam.csp_ids[icell];
                        const idx_t snode = data_->src_.csp2node[subcell];
                        const auto src_neighbours = get_node_neighbours(src_mesh_, snode, w_node);
                        triplets_size += 2 * src_neighbours.size() + 1; 
                    }
                }
        ...
        for (idx_t tnode = 0; tnode < n_tpoints_; ++tnode) {
            ...
            for (idx_t isubcell = 0; isubcell < tgt_node2csp[tnode].size(); ++isubcell) {
                ...
                for (idx_t icell = 0; icell < iparam.csp_ids.size(); ++icell) {
                    ...
                    const idx_t scell       = iparam.csp_ids[icell];
                    const idx_t spt         = src_cell_data_ ? scell : data_->src_.csp2node[scell];
                    const PointXYZ& Cs      = src_points[spt]; // !!! this is NOT barycentre of the subcell in case of NodeColumns
                    ...
                    if (src_cell_data_) {
                        ...
                        idx_t snode    = data_->src_.csp2node[scell];
