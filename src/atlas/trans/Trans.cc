...
namespace {
    ...
    template<typename T> void load_builder_functionspace() { TransBuilderFunctionSpace<T>("tmp"); }
    template<typename T> void load_builder_grid() { TransBuilderGrid<T>("tmp"); }
...
Trans::Trans( Implementation* impl ) :
  impl_(impl) {
}
...
void Trans::dirtrans_wind2vordiv( const Field& gpwind,
...
void Trans::invtrans_grad( const Field& spfield,
                           Field& gradfield,
                           const eckit::Configuration& config ) const {
  impl_->invtrans_grad( spfield, gradfield, config );
}

void Trans::invtrans_grad( const FieldSet& spfields,
                           FieldSet& gradfields,
                           const eckit::Configuration& config ) const {
...
void Trans::invtrans_vordiv2wind( const Field& spvor, const Field& spdiv,
                                  Field& gpwind,
                                  const eckit::Configuration& config ) const {
...
void Trans::invtrans( const int nb_scalar_fields, const double scalar_spectra[],
                      ...
                      double gp_fields[],
                      const eckit::Configuration& config ) const {
  impl_->invtrans( nb_scalar_fields, scalar_spectra,
...
void Trans::invtrans( const int nb_vordiv_fields, const double vorticity_spectra[], const double divergence_spectra[],
                      double gp_fields[],
                      const eckit::Configuration& config ) const {
  impl_->invtrans( nb_vordiv_fields, vorticity_spectra, divergence_spectra,
                   gp_fields,
                   config );
}
 ...
 * @param nb_fields [in] Number of fields ( both components of wind count as 1 )
 */
void Trans::dirtrans( const int nb_fields, const double wind_fields[], double vorticity_spectra[], double divergence_spectra[],
                      const eckit::Configuration& config ) const {
  impl_->dirtrans( nb_fields, wind_fields, vorticity_spectra, divergence_spectra,
                   config );
}
