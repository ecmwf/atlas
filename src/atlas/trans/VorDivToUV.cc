...
namespace {
    ...
    template<typename T> void load_builder() { VorDivToUVBuilder<T>("tmp"); }
...
VorDivToUVFactory::VorDivToUVFactory(const std::string &name):
    name_(name) {
...
VorDivToUV::VorDivToUV( Implementation* impl ) :
  impl_(impl) {
}
...
VorDivToUV::VorDivToUV( const VorDivToUV& other ) :
  impl_(other.impl_) {
}
