...
module joanna_module
  use common_module
  use parallel_module
  use datastruct_module, only: create_mesh, DataStructure_type, vector_field, scalar_field, gather
...
contains
  ...
  subroutine read_joanna(filename,rtable,g)
    ...
    do jedge = 1, nedge
      ...
      if ( keep_edge(jedge).eq.1 ) then
        ...
        g%edges_glb_idx(iedge) = jedge
        g%edges_proc(iedge) = proc( node_loc_idx(ip1) )
        ...
        if( g%edges_proc(iedge) .eq. myproc ) then
  ...
  subroutine write_results_joanna(filename,rtable,g)
    ...
    call gather(loc_Q,Q, g)
    call gather(loc_coords,coords, g)
