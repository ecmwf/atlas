...
Tool::Tool( int argc, char** argv ) : AtlasTool( argc, argv ) {
    ...
    add_option( new SimpleOption<std::string>(
        "type", "Type of trans implementation (if not specified, all types are used)" ) );
...
int Tool::execute( const Args& args ) {
    ...
    Log::info() << "  Caching        : " << std::boolalpha << args.getBool( "caching", false ) << std::endl;
    Log::info() << std::endl;


    std::map<std::string, std::vector<std::string>> linalg_backends{
        {"ifs", {"lapack"}}, {"local", {"default", "eckit_linalg", "generic", "openmp", "lapack", "eigen"}}};
    ...
    for ( auto& type : types ) {
        ...
        if ( args.getBool( "caching", false ) ) {
            ...
            if ( cache_creator.supported() ) {
                auto cachefile = eckit::PathName( "leg_" + cache_creator.uid() + ".bin" );
        ...
        for ( auto backend : linalg_backends.at( type ) ) {
            ...
            if ( linalg::dense::current_backend().available() ) {
                ...
                if ( backend == "eckit_linalg" ) {
                    backend += " (" + eckit::linalg::LinearAlgebra::backend().name() + ")";
                }

                Log::info() << "type=" << std::setw( 6 ) << std::left << type;
                Log::info() << "      backend=" << std::setw( 24 ) << std::left << backend;
                Log::info() << "      invtrans: " << elapsed_seconds.count() << " s" << std::endl;
            }
        }
    }
