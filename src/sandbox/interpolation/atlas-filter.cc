...
class AtlasFilter : public AtlasTool {
    int execute(const AtlasTool::Args& args) override;
    std::string briefDescription() override { return "Filtered gridpoint-valued variable on the same grid. Using upscaling/downscaling to a coarser resolution grid."; }
...
public:
    AtlasFilter(int argc, char* argv[]): AtlasTool(argc, argv) {
        add_option(new Separator("Options: Filter"));

        add_option(new Separator("Options: Up/Down-scaling"));
...
int AtlasFilter::execute(const AtlasTool::Args& args) {
    ...
    if (args.getBool("output-gmsh", false)) {
        ...
        util::Config config(args.getSubConfiguration("gmsh"));
        output::Gmsh{src_grid.name()+"_srcfield_filtered_on_"+tgt_grid.name()+".msh", config}.write(src_field);
    }
...
int main(int argc, char* argv[]) {
    atlas::AtlasFilter tool(argc, argv);
    return tool.start();
}
