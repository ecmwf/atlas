...
operations as the \inltc{Nodes} function space, such as going 
...
\inltc{nodes} starting from a mesh. In particular, we first initialize 
...
also in this case.
%
\lstinputlisting[caption=Functionspace nodes usage (1) using C++, 
style=CStyle, label=code-nodes-C-1, firstline=1,lastline=55]{nodes.cc}
%
%
...
%
\lstinputlisting[caption=Functionspace nodes usage (2) using C++, 
style=CStyle, label=code-nodes-C-2, firstline=55,lastline=89]{nodes.cc}
%
%
...
On line 8, we define a global field, \inltc{globalField}. 
...
Note that the first argument is the input (in our case \inltc{scalarField1}),
while the second argument is the output (in our case \inltc{globalField}).
...
and check the integrity of our \inltc{scalarField1} after all the 
...
style=CStyle, label=code-nodes-C-3, firstline=89,lastline=125]{nodes.cc}
%
%
...
\inltc{scalarField1} - note that the function space knows about 
...
explained in \parte{p:utilities}, of this user-guide.
On line 42, we finalize the library as usual.
%
\lstinputlisting[caption=Functionspace nodes usage (4) using C++, 
style=CStyle, label=code-nodes-C-4, firstline=125,lastline=166]{nodes.cc}
%
\end{description}
%
...
points and the number of entries present in \inltc{globalField}. 
...
identical to the one printed previously - i.e. \inltc{scalarField1} 
...
of \inltf{atlas\_functionspace\_nodes} and \inltf{atlas\_mesh\_nodes}.
...
also in this case.
%
\lstinputlisting[caption=Functionspace nodes usage (1) using Fortran, 
style=FStyle, label=code-nodes-F-1, firstline=1,lastline=71]{nodes.F90}
%
%
\item \underline{Definition/visualization of a scalar Field}\\[0.5em]
%
...
to the two-dimensional \inltf{scalarField1} object defined in 
...
%
\lstinputlisting[caption=Functionspace nodes usage (2) using Fortran, 
style=FStyle, label=code-nodes-F-2, firstline=71,lastline=99]{nodes.F90}
%
%
\item \underline{Parallel management}\\[0.5em]
%
...
On line 11, we define a global field, \inltf{globalField}. 
This, even if the job we are running is parallel, is defined on one task 
only. A global field can be particularly useful for input/output purposes!
Note also that the construction of the global field is based on the existing 
scalar field - i.e. the global field assumes the same characteristics of 
...
Note that the first argument is the input (in our case \inltf{scalarField1}),
while the second argument is the output (in our case \inltf{globalField}).
...
and check the integrity of our \inltf{scalarField1} after 
...
%
\lstinputlisting[caption=Functionspace nodes usage (3) using Fortran, 
style=FStyle, label=code-nodes-F-3, firstline=99,lastline=137]{nodes.F90}
%
%
\item \underline{Simple operations}\\[0.5em]
%
...
\inltf{scalarField1} - note that the function space knows about 
the parallelisation, therefore there is no need to do anything 
else to obtain the correct minimum and maximum values. This will
also be true for the other operations described below.
...
be better explained in \parte{p:utilities}, of this user-guide.
...
%
\lstinputlisting[caption=Functionspace nodes usage (4) using Fortran, 
style=FStyle, label=code-nodes-F-4, firstline=137,lastline=188]{nodes.F90}
%
\end{description}
%
It is now possible to run this simple program by using 
...
identical to the one printed previously - i.e. \inltf{scalarField1} 
has not been corrupted! - while the second is different since it 
