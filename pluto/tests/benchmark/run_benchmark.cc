...
int main(int argc, char* argv[]) {
...

    {
        ...
        auto timers_final = [&] {
            ...
            std::cout << "\toffload_time     = " << offload_time << " ( " << offload_time / ref_offload_time
                      << " ref ) " << std::endl;
            std::cout << "\thost_time        = " << host_time << " ( " << host_time / time << " total_time , "
                      << host_time / ref_host_time << " ref )" << std::endl;
            std::cout << "\tdevice_time      = " << device_time << " ( " << device_time / offload_time
                      << " offload_time , " << device_time / ref_device_time << " ref ) " << std::endl;
            if (copy_time == 0. || (ref_device_time != 0 && device_time / ref_device_time > 1.1)) {
                ...
                std::cout << "\timpl_d2h_time    = " << impl_d2h_time << " ( " << impl_d2h_time / offload_time
                          << " offload_time , " << impl_d2h_time / ref_d2h_time << " ref, "
                          << d2h_volume / impl_d2h_time / 1024. << " Gb/s ) " << std::endl;
                std::cout << "\timpl_h2d_time    = " << impl_h2d_time << " ( " << impl_h2d_time / offload_time
                          << " offload_time , " << impl_h2d_time / ref_h2d_time << " ref, "
                          << h2d_volume / impl_h2d_time / 1024. << " Gb/s ) " << std::endl;
                std::cout << "\timpl_copy_time   = " << impl_copy << " ( " << impl_copy / offload_time
                          << " offload_time , " << impl_copy / ref_copy_time << " ref, "
                          << copy_volume / impl_copy / 1024. << " Gb/s ) " << std::endl;
            }
            else {
                std::cout << "\td2h_time         = " << d2h_time << " ( " << d2h_time / offload_time
                          << " offload_time , " << d2h_time / ref_d2h_time << " ref, " << d2h_volume / d2h_time / 1024.
                          << " Gb/s ) " << std::endl;
                std::cout << "\th2d_time         = " << h2d_time << " ( " << h2d_time / offload_time
                          << " offload_time , " << h2d_time / ref_h2d_time << " ref, " << h2d_volume / h2d_time / 1024.
                          << " Gb/s ) " << std::endl;
                std::cout << "\tcopy_time        = " << copy_time << " ( " << copy_time / offload_time
                          << " offload_time , " << copy_time / ref_copy_time << " ref, "
                          << copy_volume / copy_time / 1024. << " Gb/s ) " << std::endl;
            }
            std::cout << "\tunaccounted_time = " << unaccounted_time << " ( " << unaccounted_time / offload_time
                      << " offload_time )" << std::endl;
        };
