...
\inltc{nodes} starting from a mesh. In particular, we first define 
...
operations from this perspective is \inltc{HaloExchange}. This 
...
and the number of entries present in \inltc{globalField}. Note how, for 
this non-parallel example the number of mesh points differs from the number
of grid points as mentioned earlier. We then print the other two \inltc{checksum},
the first of them is identical to the one printed previously - i.e. 
\inltc{scalarField1} has not been corrupted! - while the second is 
